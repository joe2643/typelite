...
pub fn route_ask_intent(
    ...
    VoiceIntentRouter::route(VoiceRouteRequest {
        ...
        utterance: question,
        has_selected_text,
        speech_language: match speech_language {
            None => SpeechLanguageMode::Automatic,
            Some(language) => SpeechLanguageMode::Explicit(language),
        },
        flags,
    })
}
...
/// Ask request body for the active AI preset, including its extra request fields.
...
/// Ask needs an AI preset with a base URL and a model. The API key is optional.
fn should_use_byok(config: &storage::AppConfig) -> bool {
    let preset = config.active_ai_preset();
    !preset.base_url.trim().is_empty() && !preset.model.trim().is_empty()
}

fn build_ask_stt_config(config: &storage::AppConfig, api_key: String) -> SttConfig {
    SttConfig {
        api_key,
        language: config.speech_language().map(str::to_string),
        sample_rate: 16000,
    }
}
...
pub(crate) async fn start_reserved_ask_dictation(
    ...
    let result = async {
        ...
        .await
        {
            Ok(capture_ready) => {
                if let Some(warning) = crate::audio::mic_fallback_warning(
                    &capture_ready,
                    Some(config.input_device.trim()),
                ) {
                    let _ = app.emit("pipeline:warning", &warning);
                }
        ...
        let deadline_explanation_key = resolved_limit.capability.explanation_key;
...
mod tests {
    ...
    fn ask_body_uses_low_output_cap_and_no_web_search() {
        let body = build_byok_ask_body("What is Typelite?", "test-model").unwrap();
    ...
    fn selected_text_router_defaults_to_nondestructive_ask() {
        ...
        assert_eq!(
            route_ask_intent("What does this mean?", true, Some("en"), flags).kind,
        ...
        assert_eq!(
            route_ask_intent("What is Typelite?", false, Some("en"), flags).kind,
    ...
    fn byok_ask_answer_falls_back_to_reasoning_content() {
        ...
        assert_eq!(
            validate_ask_answer(&crate::llm::protocol::response_text(&body)).unwrap(),
    ...
    #[test]
    fn ask_dictation_stt_config_uses_key_and_preset_language() {
        let mut config = storage::AppConfig::default();

        let stt_config = build_ask_stt_config(&config, "sk-test".to_string());
        assert_eq!(stt_config.api_key, "sk-test");
        assert_eq!(stt_config.language, None);

        config.speech_presets[0].language = "en".to_string();
        let stt_config = build_ask_stt_config(&config, String::new());
        assert_eq!(stt_config.language.as_deref(), Some("en"));
    }

    #[test]
    fn byok_ask_errors_include_status_and_body() {

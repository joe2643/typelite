...
/// Show or hide the Dock icon (Settings → System → "Show in Dock").
///
/// On macOS every app has an *activation policy*:
/// - `ActivationPolicy::Regular` is a normal app: Dock icon, and its own menu bar when focused.
/// - `ActivationPolicy::Accessory` is a menu-bar utility: no Dock icon and no app menu bar,
///   but it can still open windows. Typelite's tray (menu-bar) icon keeps working either way.
///
/// Switching the policy while the app runs can hide its windows, so if the main window was
/// visible it is shown and focused again afterwards. `set_activation_policy` is queued on the
/// main event loop, and the re-show is queued after it so it runs once the switch is done.
/// Does nothing on other platforms.
pub(crate) fn apply_dock_visibility(app: &tauri::AppHandle, show_in_dock: bool) {
    #[cfg(target_os = "macos")]
    {
...
mod tests {
    ...
    fn cli_action_parser_supports_first_and_later_instance_arguments() {
        let toggle = vec!["/usr/bin/typelite".to_string(), "toggle".to_string()];
        let ask = vec!["Typelite.exe".to_string(), "ask".to_string()];
    ...
    #[test]
    fn cli_action_parser_does_not_hijack_urls_or_ambiguous_commands() {
        let url_arg = vec![
            "typelite".to_string(),
            "typelite://auth/callback?mode=toggle".to_string(),
        ];
        let ambiguous = vec![
            "typelite".to_string(),
            "toggle".to_string(),
        ...
        assert_eq!(parse_cli_action(&url_arg), None);
    ...
    fn main_window_config_is_glass_without_title_bar() {
        ...
        .unwrap();
        let main = tauri_config["app"]["windows"]
            .as_array()
            .unwrap()
            .iter()
        ...
        // The desktop shows through, blurred by an NSVisualEffectView.
        assert_eq!(main["transparent"].as_bool(), Some(true));
        assert_eq!(
            main["windowEffects"]["state"].as_str(),
            Some("followsWindowActiveState")
        );
        assert!(!main["windowEffects"]["effects"]
            .as_array()
            .unwrap()
            .is_empty());
        assert_eq!(tauri_config["app"]["macOSPrivateApi"].as_bool(), Some(true));
    }
    ...
    fn macos_bundle_merges_info_plist_with_local_speech_permissions() {
        ...
        // Speech recognition happens on a whisper server, so no speech-recognition permission.
        assert!(info_plist
...
fn apply_linux_workarounds() {
    #[cfg(target_os = "linux")]
    {
        ...
        let plan = linux_workaround_plan(
            &env,
            env_flag_enabled("TYPELITE_DISABLE_WEBKIT_DMABUF"),
            env_flag_enabled("TYPELITE_DISABLE_WEBKIT_COMPOSITING"),
            env_flag_enabled("TYPELITE_FORCE_GDK_X11"),
            env_flag_enabled("TYPELITE_FORCE_SOFTWARE_GL"),
        );
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let db_path = data_dir.join("typelite.db");
            // Unmute the output if a previous run crashed while it held the mute.
            audio::output_mute::init(&data_dir);
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::permissions::get_microphone_permission,
            commands::permissions::request_microphone_permission,
            commands::permissions::get_automation_permission,
            commands::permissions::request_automation_permission,

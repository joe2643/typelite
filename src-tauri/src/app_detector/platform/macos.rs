...
/// Asks macOS whether this app may send Apple Events to the app with `bundle_id`.
/// Returns 0 when allowed, -1743 when the user said no, -1744 when macOS would have to ask
/// first, and -600 when the target app is not running (macOS cannot answer then).
pub(crate) fn automation_permission_status(bundle_id: &str, ask_user_if_needed: bool) -> i32 {
